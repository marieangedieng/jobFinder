<section class="section-box futured_jobs mt-20">
    <div class="container">
        <div class="text-center">
            ...
            <p class="font-lg color-text-paragraph-2 wow animate__animated animate__fadeInUp">The trendiest jobs right now are here ! </p>
                ...
                <ul class="nav nav-tabs" role="tablist">
                    @foreach ($featuredCategories as $category)
                        <li><a class="{{ $loop->index === 0 ? 'active' : '' }}" id="nav-tab-job-{{ $category->id }}"
                                ...
                                style="{{ $loop->index === 0 ? 'color: #0093c6;' : 'color: black;' }}"
                                onfocusin="this.style.color='#0093c6'"
                                onfocusout="this.style.color='black'"
                                >
                                {{ $category->name }}
                            </a>
                        </li>
                            ...
                            @foreach ($featuredJobs as $job)
                                <div class="col-xl-3 col-lg-4 col-md-6 col-sm-12 col-12">
                                    <div class="card-grid-2 hover-up">
                                        ...
                                        <span class="card-text-price" style="color: #0093c6">{{ $job->min_salary }} - {{ $job->max_salary }} {{ config('settings.site_default_currency') }}</span>
                                        @else
                                        <span class="card-text-price">{{ $job->custom_salary }}</span>
                                        @endif
                                            <a href="{{ route('jobs.show', $job->slug) }}">
                                                <div class="btn btn-apply-now" style="color: #0093c6; background-color: #E3F2FD">
                                                    View Post</div>
                                            </a>
                                    </div>
                                </div>
